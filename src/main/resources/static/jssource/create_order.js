$(function () {
    ...
    var inputClientPhoneNumber;

    var inputArrayForValidationAfterAutoClientInserting = [];
    var inputArrayForValidation = [];
    var inputArrayClientAutoInsertDataForValidation = [];
    ...
    $(document).ready(function () {
        ...
        inputClientPhoneNumber = $('#clientPhoneNumber');

        inputArrayForValidation.push(inputClientPhoneNumber);
        ...
        inputArrayForValidationAfterAutoClientInserting.push(inputClientPhoneNumber);
        inputArrayForValidationAfterAutoClientInserting.push(inputNameDom);
        inputArrayForValidationAfterAutoClientInserting.push(inputLastNameDom);
        inputArrayForValidationAfterAutoClientInserting.push(inputMiddleNameDom);
        inputArrayForValidationAfterAutoClientInserting.push(inputCityDom);
        inputArrayForValidationAfterAutoClientInserting.push(inputWarehouseNumberDom);
    ...
    $('#clientPhoneNumber').on('change', function (e) {
        if ($(this).val() === "PICKUP") {
            $(inputArrayForValidationAfterAutoClientInserting).each(function (iK, itemV) {
    ...
    $(document).on('change' , '#paymentMethod' , function (e) {
        ...
        if ($(this).val() === "PICKUP") {
            $(inputArrayForValidation).each(function (iK, itemV) {
                $(itemV).attr('class' ,"form-control");

            });
        } else validateRequiredFieldsAfterAutoInserting();
...
//FOCUSOUT_EVENT
    ...
    $(document).on('focusout', '#orderWeight', function (e) {
        checkEmptinessInputFieldDependentOnPaymentMethodAndSetValidClassStatus($(this));
    });
    ...
    $(document).on('focusout','#clientPhoneNumber', function (e) {
        var DCode = "+" + $(this).intlTelInput("getSelectedCountryData").dialCode;


        if(inputPaymentMethodDom.val() !== 'PICKUP'){
            if ($(this).val().replace(DCode, '').replace(/\s/g, '').length === 0) {
    ...
    $(document).on('change', '#inputOrderLineProductQua', function (event) {
        ...
        if (orderLinesMap.has(Number(productOrderLineIdInputItemVal))) {
            ...
            if ((Number(her.orderLineProductDataItem.productAmount) < inputQuaFieldValue) ||
...
// }
    ...
    function validateAndCloseOrderLineList(searchList, searchInput, selectedItem) {
        ...
        $.getJSON(location.origin + "/restApi/products/" + productId).done(function (data) {
    ...
    function validateOrderLineMapItem(ordLineMItem) {
        ...
        if (ordLineMItem.orderLineProductDataItem.currency == "USD") {
            ordLineMItem.productValidPrice = (ordLineMItem.productValidPrice * appSettings.usdCurrency).toFixed(2);
        } else if (ordLineMItem.orderLineProductDataItem.currency == "EUR") {
            ordLineMItem.productValidPrice = (ordLineMItem.productValidPrice * appSettings.eurCurrency).toFixed(2);
        }
    ...
    function validateProductPricesCurrency(prod) {
        if (prod.currency == "USD") {
            prod.price = (prod.price * appSettings.usdCurrency).toFixed(2);
        } else if (prod.currency == "EUR") {
            prod.price = (prod.price * appSettings.eurCurrency).toFixed(2);
        }
    ...
    function checkEmptinessInputFieldDependentOnPaymentMethodAndSetValidClassStatus(inputFieldForValidation) {
        if ($(inputFieldForValidation).val().replace(/\s/g, '') === "" && !($(inputPaymentMethodDom).val() === "PICKUP")) {
    ...
    function validateRequiredFieldsAfterAutoInserting() {
        ...
        $(inputArrayForValidationAfterAutoClientInserting).each(function (iK, itemV) {
            if ($(itemV).val().replace(/\s/g, '') !== "") {

                itemV.attr('class', 'form-control is-valid');
            }else {
                itemV.attr('class', 'form-control is-invalid');
            }
        });

        if($(inputOrderWeight).val() === "0.0" ||
            $(inputOrderWeight).val().replace(/\s/g, '') === "" ||
            $(inputOrderWeight).val() === "0"){
            inputOrderWeight.attr('class', 'form-control is-invalid');
        }else {
            inputOrderWeight.attr('class', 'form-control is-valid');
        }
    }
